...
update department set name='HR'where id=3;
...
select first_name, email, salary from employee where department_id = 
...
select first_name from employee where id in (select employee_id 
 from manager where department_id=(select id from department 
 where name='accounts'))order by first_name;
